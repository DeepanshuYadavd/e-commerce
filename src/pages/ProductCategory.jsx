  ...
  useEffect(() => {
    const fetchProducts = async () => {
      const res = await API.get(
        `/product?category=${category}&search=${debouncedSearch}&page=${page}`
      );
                ...
                <div className="mt-4">
                  ...
                  <div className="flex justify-between">
                    <Link to={`/product/${item._id}`}>
                      <button className="bg-purple-700 px-4 py-2 rounded-xl">
                        View
                      </button>
                    </Link>
                    <button
                      onClick={() => handleAddToCart(item._id)}
                      className="border border-purple-500 px-4 py-2 rounded-xl"
                    >
                      Add
                    </button>
                  </div>
